                  ...
                  <div className="flex items-center gap-3">
                    <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-xs">
                      <div className="flex items-center gap-1 text-red-400">
                        <Heart size={10} /><span>{char.current_hp}/{char.stat_hp + char.spent_hp}</span>
                      </div>
                      <div className="flex items-center gap-1 text-blue-400">
                        <Zap size={10} /><span>{char.current_mana}/{char.stat_mana + char.spent_mana}</span>
                      </div>
                      <div className="flex items-center gap-1 text-orange-400">
                        <Swords size={10} /><span>{char.stat_attack + char.spent_attack}</span>
                      </div>
                      <div className="flex items-center gap-1 text-cyan-400">
                        <Sparkles size={10} /><span>{char.stat_spell + char.spent_spell}</span>
                      </div>
                      <div className="flex items-center gap-1 text-green-400">
                        <Wind size={10} /><span>{char.stat_agility + char.spent_agility}</span>
                      </div>
                      <div className="flex items-center gap-1 text-sky-400">
                        <Shield size={10} /><span>{char.stat_defense}</span>
                      </div>
                      <div className="flex items-center gap-1 text-teal-400">
                        <Star size={10} /><span>{char.stat_magic_resist}</span>
                      </div>
                    </div>
        ...
        {showMasterLogin && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
            <div className="bg-gray-900 rounded-xl border border-gray-700 p-6 w-full max-w-sm">
              ...
              />
              <button onClick={tryMasterLogin} className="w-full bg-amber-700 hover:bg-amber-600 text-white py-2.5 rounded-lg font-medium transition-colors">
                로그인
              </button>
